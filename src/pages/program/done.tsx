...
export default function ProgramAfterPage() {
  const [page, setPage] = useState(1)
  const { data: programs, error } = useSWR(`/api/program?status=2&page=${page - 1}`, defaultFetcher)
  ...
  return (
    <PageHead title="완료된 프로그램 - Be:MySeason" description={description}>
      <Ul>
        {programs ? (
          programs.length > 0 ? (
...
ProgramAfterPage.getLayout = function getLayout(page: ReactElement) {
  return (
    <NavigationLayout>
